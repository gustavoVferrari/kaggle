"""Funcoes auxiliares para preparar modelos base de um ensemble voting classifier.

O modulo reutiliza as configuracoes definidas em `single_model_clf.py` para
executar buscas de hiperparametros em classificadores candidatos e fornecer
instancias de modelos base que podem ser usadas em um `VotingClassifier`.
"""
...
try:
    from single_model_clf import SingleModelOrchestrator
except ImportError:
...
def voting_model(
    ...
    """Executa busca de hiperparametros para candidatos de um voting classifier.

    Para cada classificador configurado em `single_model_clf.py`, cria um
    pipeline simples, executa `GridSearchCV` ou `RandomizedSearchCV` e retorna
    os melhores parametros encontrados com os prefixos do pipeline removidos.
    ...
    Returns:
        ...
        parametros encontrados para cada classificador.
    ...
    for model_name in selected_models:
        ...
        if search_type == "grid":
            ...
            search_params = _prefix_params(step_name, param_grid)
            search = GridSearchCV(
                make_pipeline(model),
                scoring=scoring,
...
def models(model_names=None):
    """Retorna instancias padrao dos classificadores candidatos ao voting.
    ...
    Returns:
        dict: Dicionario com classificadores base indexados pelo nome do modelo.
    """
...
if __name__ == "__main__":
    print("Voting Model Classification carregado.")
